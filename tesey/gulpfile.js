...
gulp.task('sass_all', function() {
	return gulp.src(THEME_NAME + '/sass/' + THEME_NAME + '.sass')
	.pipe(gulpIf(config.css.sourcemap,sourcemaps.init()))
	.pipe(sass({
		outputStyle: 'expand',
		includePaths: require('node-bourbon').includePaths
...
gulp.task('deploy-watch', function() {
	...
	var globs = [
		...
		THEME_NAME + '/' + THEME_NAME + '.info.yml',
		THEME_NAME + '/' + THEME_NAME + '.breakpoints.yml',
		THEME_NAME + '/' + THEME_NAME + '.libraries.yml',
    	// THEME_NAME + '/sass/**',
		THEME_NAME + '/' + THEME_NAME + '.theme',
		
	];
	globs = globs.concat(current.css).concat(current.js).concat(current.sass);
	gulp.watch(globs)
